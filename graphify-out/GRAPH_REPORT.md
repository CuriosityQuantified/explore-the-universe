...
## Corpus Check
- 159 files · ~145,326 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1057 nodes · 1864 edges · 103 communities (78 shown, 25 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 124 edges (avg confidence: 0.63)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `380fab7c`
...
- PyTorch
- test_load_graph_idempotent_uses_merge_not_create
- test_main_app_has_lifespan

## God Nodes (most connected - your core abstractions)
1. `AstronomicalObject` - 35 edges
2. `ProcessingStep` - 34 edges
3. `Observation` - 28 edges
4. `get_s3_client()` - 26 edges
5. `get_database_session()` - 20 edges
6. `_make_app_with_mock_session()` - 19 edges
7. `PipelineStatus` - 18 edges
8. `ObjectClassification` - 18 edges
9. `StepStatus` - 17 edges
10. `CatalogCrossMatch` - 17 edges
...
Cohesion: 0.08
Nodes (52): _angular_separation_arcsec(), AnomalyResponse, _catalog_external_url(), ClassificationDetailResponse, ClassificationResponse, CrossMatchDetailResponse, CrossMatchResponse, get_object_classifications() (+44 more)
...
### Community 12 - "load_graph"
Cohesion: 0.20
Nodes (10): _in_bbox(), load_graph(), task, Return True if pixel (cx, cy) lies inside bbox {xmin,ymin,xmax,ymax}., MERGE all objects for an observation into the Neo4j knowledge graph. Args:…, test_in_bbox_edge_on_boundary(), test_in_bbox_false_when_centroid_outside(), test_in_bbox_true_when_centroid_inside() (+2 more)
...
### Community 14 - "get_s3_client"
Cohesion: 0.17
Nodes (13): _get_pixel_scale(), UUID, Return WCS pixel scale in arcsec/px for this observation. Recovers FITS S3 keys…, download_fits(), task, Download calibrated FITS files from MAST and upload to MinIO. Receives a pre-…, generate_tiles(), task (+5 more)
...
Cohesion: 0.20
Nodes (15): Random Forest classifier for astronomical object morphological type prediction.…, Classify Celery task: feature extraction + RF classification for every object.…, Cross-match Celery task: query all 4 catalogs in parallel per object. Seventh…, Anomaly detection Celery task: IsolationForest scoring + multi-signal flagging.…, MAST download Celery task for JWST observations. Queries the Mikulski Archive…, Cutout generation Celery task (sixth step in the 9-task pipeline chain).…, Pipeline orchestrator Celery task for JWST observations. Dispatches the full…, Bulk-load observation objects into the Neo4j knowledge graph. Tenth (final)… (+7 more)
...
### Community 81 - "api.ts"
Cohesion: 0.08
Nodes (34): DashboardClient(), DashboardClientProps, STATUS_STYLES, DashboardPage(), metadata, decompressCocoRle(), MaskOverlay(), MaskOverlayProps (+26 more)
...
Cohesion: 0.21
Nodes (12): _make_catalog_match(), _make_object(), _make_observation(), Offline unit tests for pipeline/tasks/load_graph.py and api/db/neo4j.py. These…, Helper: patch SessionLocal + get_driver, call load_graph.run()., Galaxy CONTAINS two stars whose centroids are inside its bbox., _run_load_graph_with_mocks(), test_load_graph_contains_inference() (+4 more)

### Community 85 - "classify_objects"
Cohesion: 0.25
Nodes (9): _build_feature_matrix(), classify_objects(), _download_cutout(), ndarray, task, Extract morphological features and classify objects using a trained ML model.…, Download cutout.fits from MinIO and return its data as a float64 array., Convert list of feature dicts to a numeric matrix (n_objects × n_features). (+1 more)
...
### Community 96 - "neo4j_unit"
Cohesion: 0.50
Nodes (4): neo4j_unit, test_close_driver_nils_module_state(), test_get_driver_lazily_initialises(), test_init_driver_creates_driver_and_applies_constraints()
...
## Knowledge Gaps
- **192 isolated node(s):** `graph-refresh.sh script`, `graphify-mcp`, `explore-the-universe`, `eslintConfig`, `nextConfig` (+187 more)
...
- **Why does `AstronomicalObject` connect `AstronomicalObject` to `detect_sources.py`, `segment_sam.py`, `_make_app_with_mock_session`, `test_object_detail_api.py`, `get_database_session`, `test_classification_schema.py`, `observations.py`, `models.py`?**
