# Graph Report - explore-the-universe  (2026-09-19)

## Corpus Check
- 185 files · ~162,078 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1402 nodes · 2833 edges · 120 communities (96 shown, 24 thin omitted)
- Extraction: 90% EXTRACTED · 9% INFERRED · 0% AMBIGUOUS · INFERRED: 269 edges (avg confidence: 0.88)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `c79cbcb2`
...
- ingest_observation
- load_graph
- test_batch_cli_completes_with_postgres_numeric_storage_total
- Settings
- test_cross_match_catalogs_creates_crossmatch_records_for_real_matches

## God Nodes (most connected - your core abstractions)
1. `AstronomicalObject` - 69 edges
2. `Observation` - 40 edges
3. `_make_app_with_mock_session()` - 34 edges
4. `ProcessingStep` - 33 edges
5. `_make_chained_mock()` - 32 edges
6. `PipelineStatus` - 31 edges
7. `TestStructuredSearch` - 31 edges
8. `ObjectClassification` - 29 edges
9. `get_s3_client()` - 29 edges
10. `get_database_session()` - 28 edges
...
- `test_cross_match_catalogs_not_implemented_removed()` --indirect_call--> `cross_match_catalogs()`  [INFERRED]
  tests/test_classification_schema.py → pipeline/tasks/cross_match_catalogs.py
- `test_cross_match_catalogs_uses_thread_pool()` --indirect_call--> `cross_match_catalogs()`  [INFERRED]
  tests/test_classification_schema.py → pipeline/tasks/cross_match_catalogs.py
...
Cohesion: 0.18
Nodes (16): Phase 5 Context: Classification & Cross-Matching, Anomaly Sensitivity Decisions, Classification Taxonomy Decisions, Cross-Match Behavior Decisions, Result Storage & API Decisions, Phase 5 Research: Classification & Cross-Matching, Gaia DR3 Catalog, joblib (+8 more)
...
Cohesion: 0.16
Nodes (20): _build_neighbors(), get_graph_neighbors(), GraphCatalogEntry, GraphNeighborNode, GraphNeighborsResponse, GraphQueryResponse, GraphQueryResult, BaseModel (+12 more)
...
### Community 10 - "test_catalog.py"
Cohesion: 0.24
Nodes (16): Image, field_of_view(), image_bytes(), import_image(), make_assets(), prepare_images(), Validate a real 2-D celestial FITS image and stretch it for display., Offline regression tests for real catalog parsing and bounded imagery imports. (+8 more)
...
Cohesion: 0.08
Nodes (86): get_database_session(), TestClient, _make_app_with_mock_session(), Regression suite: Phase 5 Plan 3 — anomaly API endpoints. All tests are offline…, Returns [] (not 404) when no anomaly-flagged objects exist., Return a TestClient wired with a mock DB session override., test_anomalies_returns_empty_list_when_none_found(), test_anomalies_returns_flagged_objects_with_explanation() (+78 more)
...
### Community 14 - "_process_fits_to_tiff"
Cohesion: 0.17
...
Cohesion: 0.06
Nodes (13): Unit tests for Phase 5 Plan 1: classification schema, catalog clients, and…, generate_cutouts must NOT assign pipeline_status = PipelineStatus.completed., load_or_create_classifier returns None (not an exception) when S3 has no model., Catalog failure → no CatalogCrossMatch, task continues without aborting., test_classify_objects_not_implemented_removed(), test_cross_match_catalogs_not_implemented_removed(), test_cross_match_catalogs_not_queried_on_catalog_failure(), test_cross_match_catalogs_uses_thread_pool() (+5 more)
...
### Community 35 - "catalog_import.py"
Cohesion: 0.21
Nodes (15): coordinate(), diverse_order(), fetch_bytes(), identity(), load_targets(), UUID, Bounded, resumable OpenNGC + DSS2 import. Run with python -m…, Import genuine published records idempotently, without fabricated detections. (+7 more)
...
Cohesion: 0.17
Nodes (15): _load(), parametrize, Path, Guards on the committed graphify knowledge-graph integration. These are static…, graph.json is committed, so it must not embed this checkout's location., The graph only stays fresh if this hook survives; `graphify install` rewrites…, CI (.github/workflows/knowledge-graph.yml) shells out to this., `graphify install` hardcodes an absolute interpreter path here. That path only… (+7 more)

### Community 60 - "cross_match_catalogs"
Cohesion: 0.29
Nodes (8): cross_match_catalogs(), _get_pixel_scale(), SkyCoord, task, UUID, _query_one_catalog(), Cross-match detected objects against SIMBAD, NED, SDSS, and Gaia. Seventh step…, Return WCS pixel scale in arcsec/px for this observation. Recovers FITS S3 keys…
...
Cohesion: 0.16
Nodes (22): Tile serving API endpoints for the sky viewer. GET…, Classify Celery task: feature extraction + RF classification for every object.…, Cross-match Celery task: query all 4 catalogs in parallel per object. Seventh…, download_fits(), task, MAST download Celery task for JWST observations. Queries the Mikulski Archive…, Download calibrated FITS files from MAST and upload to MinIO. Receives a pre-…, Cutout generation Celery task (sixth step in the 9-task pipeline chain).… (+14 more)

### Community 65 - "CatalogBrowser.tsx"
Cohesion: 0.17
Nodes (15): BrowserState, CatalogBrowser(), column(), goToPage(), jumpToPage(), sortBy(), update(), initialState (+7 more)
...
Cohesion: 0.06
Nodes (61): chat_query(), ChatContext, ChatObjectResult, ChatRequest, ChatResponse, BaseModel, post, Session (+53 more)

### Community 72 - "routers/ingest.py"
Cohesion: 0.17
Nodes (15): get_ingest_status(), IngestRequest, IngestResponse, IngestStatusResponse, BaseModel, get, post, Session (+7 more)

### Community 73 - "detect_sources.py"
Cohesion: 0.13
...
### Community 74 - "ProcessingStep"
Cohesion: 0.13
Nodes (19): DeclarativeBase, IsolationForest, _build_feature_matrix(), detect_anomalies(), _finalize_empty(), _impute_sentinels(), ndarray, task (+11 more)
...
### Community 78 - "compute_search_radius_arcsec"
Cohesion: 0.25
...
Cohesion: 0.16
Nodes (15): _make_app_with_mock_session(), _make_obj(), _mock_session_with_objects(), Tests for SIMBAD name-search mode of GET /api/objects/search. Covers: -…, AC2: SIMBAD resolves but no local objects within 5 arcsec → empty results, not…, AC2 variant: SIMBAD returns None (name not found) → empty results with null…, AC3: SIMBAD raises RuntimeError (unreachable) → 503 with error detail., Multiple local objects within 5 arcsec are all returned. (+7 more)
...
Cohesion: 0.13
Nodes (14): neo4j_unit, _in_bbox(), Return True if pixel (cx, cy) lies inside bbox {xmin,ymin,xmax,ymax}., Offline unit tests for pipeline/tasks/load_graph.py and api/db/neo4j.py. These…, Source check: load_graph must not use CREATE for nodes (only MERGE)., api/main.py must wire init_driver/close_driver via a lifespan context., test_close_driver_nils_module_state(), test_get_driver_lazily_initialises() (+6 more)
...
Cohesion: 0.11
Nodes (24): react-force-graph, AI-assisted Natural Language Querying, Anomaly Detection and Novel Object Flagging, Anti-feature: Full Citizen Science Platform, Anti-feature: Full LSST Real-time Ingestion, Anti-feature: Raw SQL/ADQL Query Interface, Anti-feature: Spectral Analysis Tools, Catalog Cross-matching (+16 more)
...
### Community 89 - "ObjectClassification"
Cohesion: 0.18
Nodes (11): ObjectClassification, Append-only ML classification record for a single pipeline run on one object., classify_objects creates ObjectClassification for objects without SAM masks., When no ML model exists in S3, all objects get predicted_type='unknown',…, ObjectClassification.feature_vector is a non-empty dict (JSONB payload)., test_classify_objects_creates_classification_for_maskless_object(), test_classify_objects_no_model_classifies_as_unknown(), test_classify_objects_stores_feature_vector_jsonb() (+3 more)

### Community 90 - "Astropy"
Cohesion: 0.50
Nodes (4): Astropy, astroquery, FITS File Support, reproject
...
Cohesion: 0.25
Nodes (9): SearchResults(), run(), QueryBuilder(), handleSearch(), searchByCone(), searchByFilters(), searchByName(), searchByType() (+1 more)
...
### Community 102 - "get_observation_detail"
Cohesion: 0.13
...
### Community 103 - "fetchObjectTypes"
Cohesion: 0.27
Nodes (4): CatalogSummary(), SearchBar(), Tab, fetchObjectTypes()
...
Cohesion: 0.30
Nodes (10): ViewerPage(), ViewerPageProps, QueryBuilderProps, fetchObservation(), fetchWcsParams(), getTileUrl(), NameSearchResult, ObjectSearchItem (+2 more)

### Community 106 - "FastAPI"
Cohesion: 0.17
Nodes (13): FastAPI, browser(), fixture, parametrize, Offline validation and response-contract tests for catalog browsing., test_invalid_filters_rejected_before_database_query(), catalog(), names() (+5 more)

### Community 107 - "main.py"
Cohesion: 0.27
Nodes (9): close_driver(), get_driver(), init_driver(), Neo4j driver singleton and FastAPI dependency. One driver is created at FastAPI…, Create the singleton driver and apply schema constraints., Close the singleton driver (called at FastAPI shutdown)., Return the singleton driver, initialising lazily if needed., lifespan() (+1 more)

### Community 108 - "routers/catalog.py"
Cohesion: 0.29
Nodes (10): browse_objects(), catalog_summary(), CatalogEntry, CatalogPage, BaseModel, get, Session, Paginated object browsing and live catalog coverage. (+2 more)

### Community 109 - "catalog-ingestion.md"
Cohesion: 0.33
Nodes (4): Catalog imagery on Railway, Storage and operation, Railway deployment, Troubleshooting

### Community 110 - "AstronomicalObject"
Cohesion: 0.18
Nodes (11): AstronomicalObject, Observations with < 10 objects must skip IsolationForest gracefully., Objects whose predicted_object_type == 'artifact' must NOT be flagged., anomaly_explanation must be a non-empty string when signals fire., test_astronomical_object_catalog_object_name_is_indexed(), test_astronomical_object_has_catalog_magnitude(), test_astronomical_object_has_catalog_object_name(), test_astronomical_object_has_catalog_redshift() (+3 more)

### Community 111 - "_run_load_graph_with_mocks"
Cohesion: 0.25
...
### Community 112 - "Architecture Research"
Cohesion: 0.22
Nodes (9): Architecture Research, Dual Database Strategy (pattern), ML Classifier (component), Neo4j Knowledge Graph (component), Pipeline-as-DAG with Celery Chains (pattern), Pipeline Orchestrator (component), PostgreSQL + PostGIS (component), SAM Segmenter (component) (+1 more)

### Community 113 - "PipelineStatus"
Cohesion: 0.29
Nodes (7): post, Session, Temporary test endpoint: uploads a file to MinIO and creates an observation…, upload_test_file(), PipelineStatus, str, UploadFile

### Community 114 - "revision.py"
Cohesion: 0.60
Nodes (3): get_revision(), get, test_revision_endpoint_returns_configured_revision()

### Community 115 - "ingest_observation"
Cohesion: 0.40
...
### Community 116 - "load_graph"
Cohesion: 0.40
Nodes (5): load_graph(), task, MERGE all objects for an observation into the Neo4j knowledge graph. Args:…, test_load_graph_uses_merge(), test_load_graph_uses_unwind()

### Community 117 - "test_batch_cli_completes_with_postgres_numeric_storage_total"
Cohesion: 0.50
Nodes (4): main(), parametrize, Postgres SUM(bigint) returns Decimal; completion must emit exact JSON integers., test_batch_cli_completes_with_postgres_numeric_storage_total()
...
## Knowledge Gaps
- **207 isolated node(s):** `graph-refresh.sh script`, `graphify-mcp`, `explore-the-universe`, `eslintConfig`, `nextConfig` (+202 more)
...
- **What is the exact relationship between `Next.js Web README` and `window.svg Icon`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._
- **Why does `Project Research Summary` connect `Project Research Summary` to `Next.js Web README`, `Image Tiling (HiPS/HEALPix)`, `Astropy`, `Knowledge Graph With Spatial Hierarchy`?**
  _High betweenness centrality (0.118) - this node is a cross-community bridge._
- **Why does `AstronomicalObject` connect `AstronomicalObject` to `test_catalog.py`, `segment_sam.py`, `TestClient`, `test_classification_schema.py`, `_make_app_with_mock_session`, `catalog_import.py`, `cross_match_catalogs`, `Observation`, `objects.py`, `detect_sources.py`, `ProcessingStep`, `_make_app_with_mock_session`, `observations.py`, `_make_app_with_mock_session`, `test_observations_api.py`, `_make_app_with_mock_session`, `generate_cutouts`, `classify_objects`, `FastAPI`, `routers/catalog.py`, `load_graph`?**
